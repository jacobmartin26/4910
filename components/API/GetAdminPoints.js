...
const GetReasons = () => {
    ...
    const handleDriverChange = (event) => {
        ...
        const selectedDriver = drivers.find(driver => driver.user_id === selectedDriverId);
        //console.log("Company Name:", selectedDriver ? selectedDriver.company_name : "No company found");
    ...
    const formInputChange = (formField, value) => {
      ...
      const handleFormSubmit = (values) => {
        ...
        setLoading(false)
        navigate('/pages/points/admin');
      };

      const handleFormSubmit2 = (values) => {
        setLoading(true);
        
            API.post('add/reason/admin/' + selectedCompanyId, {
                reason : reason,
              });
            ...
            <select value={selectedReason} onChange={handleReasonChange}>
                ...
                {reasons && reasons.map((app, index) => (
                    <option key={index} value={app.reason_id}>
                        {app.reason} (Company : {app.company_name})
                    </option>
                ))}
            </select>
            <div>
                    ...
                    {drivers.map((driver, index) => (
                        <li key={index}>
                            ...
                            <label htmlFor={`driver${driver.user_id}`}>
                                {driver.fname} {driver.lname} ({driver.company_name} : {driver.points} points)
                            </label>
                        </li>
                    ))}
                </ul>
            ...
            <div>
                ...
                <br/>
                    <TextField
                        ...
                        sx={{ mb: 3 }}
                    />
                <H3>Please enter for which company:</H3>
                <br/>
